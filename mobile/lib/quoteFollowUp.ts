...
export type QuoteFollowUpConfig = { enabled: boolean; days: number }
...
export function mergeQuoteFollowUpPreferences(preferences: unknown, config: QuoteFollowUpConfig): JsonObject {
  const root = isObject(preferences) ? preferences : {}
  const current = isObject(root.quote_follow_up) ? root.quote_follow_up : {}
  return {
    ...root,
    quote_follow_up: { ...current, enabled: Boolean(config.enabled), days: normalizeDays(config.days) },
  }
}
