...
// contractor's services, 24h strip, 7-day cards. GPS re-pin via
    ...
    <View style={{ flex: 1 }}>
      ...
      <ScrollView contentContainerStyle={{ paddingTop: insets.top + 10, paddingBottom: insets.bottom + 24, paddingHorizontal: 20 }}>
        <ScreenHeader backLabel="Home" onBack={() => router.back()} eyebrow="Pour Window" title="Forecast"
          right={<Pressable onPress={pinLocation} disabled={pinning} style={{ width: 44, height: 44, borderRadius: 14, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: hasCoords ? theme.borderGold : theme.borderMid, backgroundColor: hasCoords ? `${theme.goldBright}1a` : theme.surface2 }}>{pinning ? <ActivityIndicator size="small" color={theme.goldBright} /> : <MapPin color={hasCoords ? theme.goldBright : theme.ink} size={20} />}</Pressable>} />

        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, alignSelf: 'flex-start', paddingHorizontal: 10, paddingVertical: 5, borderRadius: 999, backgroundColor: theme.surface2, borderWidth: 1, borderColor: theme.borderMid, marginTop: 14 }}>
          <MapPin color={theme.goldBright} size={11} />
          <Text style={{ color: theme.inkMuted, fontSize: 11, fontWeight: '600' }}>{cityName || (hasCoords ? `${lat.toFixed(2)}, ${lon.toFixed(2)}` : 'Murfreesboro, TN')}</Text>
        </View>
        ...
        <Card glow accent={tone.fg} style={{ marginTop: 14, marginBottom: 14 }}>
          <View style={{ padding: 20, paddingLeft: 22 }}>
            <View style={{ flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'space-between' }}>
              <View style={{ flex: 1 }}>
                <Text style={{ color: theme.inkMuted, fontSize: 10, fontWeight: '700', letterSpacing: 1.8, textTransform: 'uppercase' }}>Today · {weatherLabel(c?.weather_code)}</Text>
                <View style={{ flexDirection: 'row', alignItems: 'flex-start', marginTop: 8 }}>
                  <Text style={{ color: theme.ink, fontSize: 64, fontWeight: '800', lineHeight: 64, letterSpacing: -1 }}>{loading ? '—' : c?.temperature_2m != null ? Math.round(c.temperature_2m) : '—'}</Text>
                  <Text style={{ color: theme.inkMuted, fontSize: 24, fontWeight: '700', marginTop: 6 }}>°F</Text>
                </View>
              </View>
              <View style={{ width: 54, height: 54, borderRadius: 16, alignItems: 'center', justifyContent: 'center', backgroundColor: '#1a2535' }}>
                <CloudSun color="#8fb4e3" size={26} />
              </View>
            </View>

            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, marginTop: 16, padding: 12, borderRadius: 14, backgroundColor: tone.bg, borderWidth: 1, borderColor: tone.border }}>
              <View style={{ width: 10, height: 10, borderRadius: 5, backgroundColor: tone.fg }} />
              <View style={{ flex: 1 }}>
                <Text style={{ color: tone.fg, fontSize: 10, fontWeight: '800', letterSpacing: 1.4, textTransform: 'uppercase' }}>{currentWindow.label || tone.label}</Text>
        ...
        {strip.length > 0 ? (
          <View style={{ marginBottom: 18 }}>
            <Text style={{ color: theme.inkMuted, fontSize: 10, fontWeight: '800', letterSpacing: 2, textTransform: 'uppercase', marginBottom: 10 }}>Next 24 hours</Text>
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 6 }}>
              {strip.map((h: any, i: number) => {
                const t = toneOf(h.status)
                return (
                  <View key={i} style={{ width: 50, paddingVertical: 10, borderRadius: 12, backgroundColor: theme.surface2, borderWidth: 1, borderColor: theme.borderMid, alignItems: 'center', gap: 8 }}>
                    <Text style={{ color: theme.inkMuted, fontSize: 10, fontWeight: '700' }}>{fmtHour(h.time)}</Text>
                    ...
                    <Text style={{ color: theme.inkFaint, fontSize: 9 }}>{Math.round(h.rain || 0)}%</Text>
                  </View>
                )
              })}
            </ScrollView>
        ...
        {daily.length > 0 ? (
          <View style={{ marginBottom: 18 }}>
            <Text style={{ color: theme.inkMuted, fontSize: 10, fontWeight: '800', letterSpacing: 2, textTransform: 'uppercase', marginBottom: 10 }}>Forecast</Text>
            <View style={{ gap: 8 }}>
              {daily.map((d: any, i: number) => {
                ...
                return (
                  <Card key={d.time} accent={t.fg}>
                    <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', padding: 14, paddingLeft: 18 }}>
                      <View style={{ flex: 1 }}>
                        <View style={{ flexDirection: 'row', alignItems: 'baseline', gap: 8 }}>
                          <Text style={{ color: i === 0 ? theme.goldBright : theme.ink, fontSize: 15, fontWeight: '800', letterSpacing: 1 }}>{i === 0 ? 'TODAY' : fmtDay(d.time)}</Text>
                          <Text style={{ color: theme.inkMuted, fontSize: 11 }}>{fmtDate(d.time)}</Text>
                        </View>
                        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, marginTop: 5 }}>
                          <Text style={{ color: theme.inkMuted, fontSize: 11 }}>💧 {Math.round(d.rainPct)}%</Text>
                          <Text style={{ color: theme.inkMuted, fontSize: 11 }}>💨 {Math.round(d.windMax)} mph</Text>
                          <View style={{ paddingHorizontal: 8, paddingVertical: 2, borderRadius: 999, backgroundColor: t.bg, borderWidth: 1, borderColor: t.border }}>
                            <Text style={{ color: t.fg, fontSize: 9, fontWeight: '800', letterSpacing: 1 }}>{ds === 'go' ? 'GO' : ds === 'warn' ? 'TIGHT' : 'STOP'}</Text>
                          </View>
                        </View>
                      </View>
                      <View style={{ alignItems: 'flex-end' }}>
                        <Text style={{ color: theme.ink, fontSize: 20, fontWeight: '800' }}>{fmtTemp(d.tMax)}</Text>
                        <Text style={{ color: theme.inkFaint, fontSize: 13, fontWeight: '700' }}>{fmtTemp(d.tMin)}</Text>
                      </View>
                    </View>
                  </Card>
                )
        ...
        {/* Per-trade */}
        <Text style={{ color: theme.inkMuted, fontSize: 10, fontWeight: '800', letterSpacing: 2, textTransform: 'uppercase', marginBottom: 10 }}>Trades on your list · today</Text>
        {tradeRows.length === 0 ? (
          <Text style={{ color: theme.inkMuted, fontSize: 13 }}>Awaiting current conditions.</Text>
        ) : (
          <View style={{ gap: 6 }}>
            {tradeRows.map((r: any) => {
              ...
              return (
                <View key={r.trade} style={{ flexDirection: 'row', alignItems: 'center', gap: 10, padding: 12, borderRadius: 12, backgroundColor: theme.surface2, borderWidth: 1, borderColor: theme.borderMid }}>
                  <View style={{ width: 28, height: 28, borderRadius: 8, alignItems: 'center', justifyContent: 'center', backgroundColor: t.bg, borderWidth: 1, borderColor: t.border }}>
                    <Glyph color={t.fg} size={14} strokeWidth={2.4} />
                  </View>
                  <View style={{ flex: 1 }}>
                    <Text style={{ color: theme.ink, fontSize: 13, fontWeight: '700', textTransform: 'capitalize' }}>{r.trade}</Text>
                    <Text style={{ color: r.status === 'go' ? theme.inkFaint : theme.inkMuted, fontSize: 11, marginTop: 2 }}>{r.status !== 'go' && r.reasons?.length ? r.reasons[0] : 'Within spec'}</Text>
                  </View>
                  <View style={{ paddingHorizontal: 9, paddingVertical: 3, borderRadius: 999, backgroundColor: t.bg, borderWidth: 1, borderColor: t.border }}>
                    <Text style={{ color: t.fg, fontSize: 9, fontWeight: '800', letterSpacing: 1.2 }}>{r.status === 'go' ? 'GO' : r.status === 'warn' ? 'TIGHT' : 'STOP'}</Text>
                  </View>
                </View>
              )
            })}
...
function Metric({ Icon, label, value, unit }: { Icon: any; label: string; value: string; unit: string }) {
  return (
    <View style={{ flex: 1, padding: 10, borderRadius: 12, backgroundColor: theme.surface2, borderWidth: 1, borderColor: theme.borderMid, gap: 6 }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
        <Icon color={theme.inkMuted} size={10} />
        <Text style={{ color: theme.inkMuted, fontSize: 9, fontWeight: '700', letterSpacing: 1, textTransform: 'uppercase' }}>{label}</Text>
      </View>
      <View style={{ flexDirection: 'row', alignItems: 'baseline', gap: 4 }}>
        <Text style={{ color: theme.ink, fontSize: 22, fontWeight: '800' }}>{value}</Text>
        <Text style={{ color: theme.inkFaint, fontSize: 10, fontWeight: '700', textTransform: 'uppercase' }}>{unit}</Text>
      </View>
    </View>
  )
}
