...
export default function LoginScreen() {
  ...
  return (
    ...
    >
      ...
      >
        <Text className="text-gold-bright text-xs font-bold tracking-[2px] uppercase mb-2">
          FieldHorse
        </Text>
        ...
        <TextInput
          ...
          className="bg-surface border border-[rgba(242, 237, 228,0.12)] rounded-[10px] px-4 py-3 text-ink mb-5"
        />

        <Text className="text-ink-muted text-xs font-bold tracking-[0px] uppercase mb-2">Password</Text>
        <TextInput
          value={password}
        ...
        {error ? <Text className="text-[#C9963A] text-sm mb-4">{error}</Text> : null}
        {notice ? <Text className="text-[#5C5C5C] text-sm mb-4">{notice}</Text> : null}

        <Pressable
          onPress={handleSignIn}
          disabled={busy}
          className="rounded-[10px] py-4 items-center"
          style={{ backgroundColor: busy ? 'rgba(201, 150, 58,0.5)' : '#C9963A' }}
        >
          {busy
            ? <ActivityIndicator color="#141414" />
            : <Text className="text-[#141414] text-base font-bold">Sign in</Text>}
        </Pressable>
