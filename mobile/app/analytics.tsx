...
const STAGE_TINT: Record<string, string> = {
  lead: '#5C5C5C', quote: '#C9963A', job: '#2D7A4F', invoice: '#C9963A', closed: '#5C5C5C', lost: '#C0392B'
}
                ...
                <Card>
                  <View style={{ padding: 16 }}>
                      ...
                      {stats.monthly.map((m, i) => {
                        ...
                        return (
                          <View key={i} style={{ flex: 1, alignItems: 'center', justifyContent: 'flex-end' }}>
                            {m.value > 0 ? <Text style={{ color: theme.inkMuted, fontSize: 12, fontWeight: '700', marginBottom: 4 }}>{money(m.value)}</Text> : null}
                            <View style={{ width: '70%', height: h, borderRadius: 10, backgroundColor: m.value > 0 ? theme.goldBright : 'rgba(242, 237, 228,0.08)' }} />
                          </View>
                        )
                      })}
                    </View>
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', gap: 8, marginTop: 8 }}>
                      {stats.monthly.map((m, i) => (
                        <Text key={i} style={{ flex: 1, textAlign: 'center', color: theme.inkMuted, fontSize: 12, fontWeight: '700' }}>{m.label}</Text>
                      ))}
                    </View>
                  </View>
                </Card>
                    ...
                    {stats.winByType.map((r) => (
                      <View key={r.type}>
                        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 5 }}>
                          <Text style={{ color: theme.ink, fontSize: 14, fontWeight: '700' }}>{r.type} <Text style={{ color: theme.inkMuted }}>· {r.won}/{r.total} won</Text></Text>
                          <Text style={{ color: theme.goldBright, fontSize: 14, fontWeight: '800' }}>{r.rate}%</Text>
                        </View>
                        <View style={{ height: 8, borderRadius: 10, backgroundColor: 'rgba(242, 237, 228,0.06)', overflow: 'hidden' }}>
                          <View style={{ width: `${r.rate}%`, height: 8, borderRadius: 10, backgroundColor: theme.success }} />
                        </View>
                      </View>
                    ))}
                  </View>
                ...
                <Card>
                  <View style={{ padding: 16, gap: 12 }}>
                    {stats.topClients.map((c) => (
                      <View key={c.id}>
                        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 5 }}>
                          <Text style={{ color: theme.ink, fontSize: 14, fontWeight: '700' }} numberOfLines={1}>{c.name}</Text>
                          <Text style={{ color: theme.goldBright, fontSize: 14, fontWeight: '800' }}>{money(c.value)}</Text>
                        </View>
                        <View style={{ height: 8, borderRadius: 10, backgroundColor: 'rgba(242, 237, 228,0.06)', overflow: 'hidden' }}>
                          <View style={{ width: `${Math.round((c.value / maxClientValue) * 100)}%`, height: 8, borderRadius: 10, backgroundColor: theme.goldBright }} />
                        </View>
                      </View>
                    ))}
                  </View>
              ...
              <View style={{ padding: 16, gap: 12 }}>
                {STAGES.map((s) => {
                  ...
                  return (
                    <View key={s}>
                      <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 5 }}>
                        <Text style={{ color: theme.ink, fontSize: 14, fontWeight: '700', textTransform: 'capitalize' }}>{s} <Text style={{ color: theme.inkMuted }}>· {row.count}</Text></Text>
                        <Text style={{ color: theme.inkMuted, fontSize: 14 }}>{money(row.value)}</Text>
                      </View>
                      <View style={{ height: 8, borderRadius: 10, backgroundColor: 'rgba(242, 237, 228,0.06)', overflow: 'hidden' }}>
                        <View style={{ width: `${pct}%`, height: 8, borderRadius: 10, backgroundColor: tint }} />
                      </View>
                    </View>
                  )
                })}
